...
* `topic.id` Indicates that the API response should only contain resources associated with the specified topic. The value identifies a [Freebase topic ID](https://developers.google.com/freebase/v1/topic-overview).
* `verbose` If `TRUE` prints infromational messages in the console. The default value is `FALSE`.
...
video.cat <- video.cat[grep("Pets", video.cat$snippet.title),] 
