...
graphTweets let's you build a temporal graph in R and easily export to/open it in [Gephi](https://gephi.org/); you only need tweets and `dynamise`. 
...
`dynamise` returns an object of class `graph` and though therefore can be plotted using `plot` it will not be dynamic. To take view the dynamic graph the network must be open in Gephi. Thankfully the function makes it extremely easy.
...
In the following window check "parse dates", if you followed the steps in the previous slides in creating the graph and set `start.stamp = "created"` in `dynamise` use <b>yyyy-MM-dd HH:mm:ss</b> as date format (this is essentially `POSIXct`/`POSIXlt` class in R)
