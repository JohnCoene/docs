...
bks$colour <- ifelse(bks$Sentiment >= 0, "#5dd39e", "#348aa7")
...
ggplot(data = bks, aes(x = Book.Number, y = Sentiment)) +
  ...
  scale_fill_manual(values = c("#5dd39e", "#348aa7")) +
...
comparison.cloud(term.matrix, max.words = 200, random.order = FALSE,
                 rot.per = 0.25, colors = rev(c("#5dd39e", "#348aa7")), title.size = 1, 
                 scale = c(3, .5))
```
...
pairs <- pairs[1:250,] # take most freq pairs
...
plot(wc, g, layout = igraph::layout.fruchterman.reingold(g, niter=10000), 
     ...
     edge.color = hsv(h = 0, s = 0, v = 0, alpha = 0.7),
